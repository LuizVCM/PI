...
export class Migration1790281446663 implements MigrationInterface {
    name = 'Migration1790281446663'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`plantas\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`nome\` varchar(100) NOT NULL, \`nomeCientifico\` varchar(150) NOT NULL, \`categoria\` enum ('cereais', 'leguminosas', 'tubérculos', 'hortaliças', 'frutas', 'oleaginosas', 'fibras', 'forrageiras', 'estimulantes', 'adoçantes', 'industriais') NULL, \`cicloMinimoDias\` int NOT NULL, \`cicloMaximoDias\` int NOT NULL, \`phMinimo\` decimal(4,2) NULL, \`phMaximo\` decimal(4,2) NULL, \`temperaturaMinima\` decimal(5,2) NULL, \`temperaturaMaxima\` decimal(5,2) NULL, \`precipitacaoMinima\` decimal(6,2) NULL, \`precipitacaoMaxima\` decimal(6,2) NULL, \`necessidadeLuz\` varchar(50) NULL, \`necessidadeAgua\` varchar(50) NULL, \`texturaSolo\` varchar(100) NULL, \`kcMedio\` decimal(6,4) NULL, \`nitrogenio\` decimal(8,2) NULL, \`fosforo\` decimal(8,2) NULL, \`potassio\` decimal(8,2) NULL, \`unidadeNpk\` enum ('mg/kg', 'ppm', '%') NULL, UNIQUE INDEX \`IDX_d649e3616e52585a18bd710263\` (\`nomeCientifico\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`estoque_insumos\` (\`id\` int NOT NULL AUTO_INCREMENT, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`nome\` varchar(100) NOT NULL, \`categoria\` enum ('fertilizantes', 'defensivos', 'ferramentas') NOT NULL, \`quantidade\` decimal(5,2) NOT NULL, \`unidade\` enum ('g', 'kg', 'ton', 'ml', 'l', 'sacas', 'un') NOT NULL, \`dataValidade\` date NULL, \`limiteMinimo\` decimal(5,2) NULL, \`usuarioId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
